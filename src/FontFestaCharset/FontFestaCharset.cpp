...
int UMain(int argc, UChar* argv[])
{
	...
	u32 uXInCell = SToU32(argv[9]);
	u32 uYInCell = SToU32(argv[10]);
	...
	delete[] pGfd;
	fp = UFopen(argv[7], USTR("rb"), false);
	if (fp == nullptr)
	{
		return 1;
	}
	...
	fseek(fp, 0, SEEK_END);
	u32 uFont1Size = ftell(fp);
	fseek(fp, 0, SEEK_SET);
	pFont = new u8[uFont1Size];
	fread(pFont, 1, uFont1Size, fp);
	fclose(fp);
	if (!code32BPPTex(pFont, true))
	{
		delete[] pFont;
	...
	STexHeader* pFont1Header = reinterpret_cast<STexHeader*>(pFont);
	u32 uHeader1Size = sizeof(STexHeader) + 4 + pFont1Header->MipmapLevel * 4;
	pTexHeader = reinterpret_cast<STexHeader*>(&*mTexFileOld[1].begin());
	for (u32 i = 0; i < pFont1Header->Height; i++)
	{
		memcpy(&*mTexFileOld[1].begin() + uHeaderSize + i * 1024 * 4, pFont + uHeader1Size + i * pFont1Header->Width * 4, pFont1Header->Width * 4);
	}
	delete[] pFont;
	...
	fseek(fp, 0, SEEK_END);
	u32 uFont2Size = ftell(fp);
	fseek(fp, 0, SEEK_SET);
	pFont = new u8[uFont2Size];
	fread(pFont, 1, uFont2Size, fp);
	fclose(fp);
	if (!code32BPPTex(pFont, true))
	{
		delete[] pFont;
	...
	STexHeader* pFont2Header = reinterpret_cast<STexHeader*>(pFont);
	u32 uHeader2Size = sizeof(STexHeader) + 4 + pFont2Header->MipmapLevel * 4;
	pTexHeader = reinterpret_cast<STexHeader*>(&*mTexFileOld[2].begin());
	for (u32 i = 0; i < pFont2Header->Height; i++)
	{
		memcpy(&*mTexFileOld[2].begin() + uHeaderSize + i * 1024 * 4, pFont + uHeader2Size + i * pFont2Header->Width * 4, pFont2Header->Width * 4);
	}
	delete[] pFont;
	fp = UFopen(argv[6], USTR("rb"), false);
	if (fp == nullptr)
	{
		return 1;
	}
	fseek(fp, 0, SEEK_END);
	u32 uFont3Size = ftell(fp);
	fseek(fp, 0, SEEK_SET);
	pFont = new u8[uFont3Size];
	fread(pFont, 1, uFont3Size, fp);
	fclose(fp);
	if (!code32BPPTex(pFont, true))
	{
		delete[] pFont;
	...
	STexHeader* pFont3Header = reinterpret_cast<STexHeader*>(pFont);
	u32 uHeader3Size = sizeof(STexHeader) + 4 + pFont3Header->MipmapLevel * 4;
	pTexHeader = reinterpret_cast<STexHeader*>(&*mTexFileOld[3].begin());
	for (u32 i = 0; i < pFont3Header->Height; i++)
	{
		memcpy(&*mTexFileOld[3].begin() + uHeaderSize + i * 1024 * 4, pFont + uHeader3Size + i * pFont3Header->Width * 4, pFont3Header->Width * 4);
	}
	delete[] pFont;
	fp = UFopen(argv[8], USTR("rb"), false);
	if (fp == nullptr)
	{
		return 1;
	}
	...
	for (u32 i = 0; i < uTexCount; i++)
	{
		UString sFileName = Format(USTR("font_festa_%02d_HQ.tex"), i);
