...
#pragma prefast(suppress:22103, "writing blocks in two halves confuses tool")
    ...
    for (size_t i = 0; i < NUM_PIXELS_PER_BLOCK; ++i)
    {
        pColor[i] = static_cast<uint8_t>(pColorInternal[i].vector4_f32[0] * 255.0f);
    }
}
...
#pragma prefast(suppress:22103, "writing blocks in two halves confuses tool")
    ...
    for (size_t i = 0; i < NUM_PIXELS_PER_BLOCK; ++i)
    {
        pColor[i * 2] = static_cast<uint8_t>(pColorInternal[i].vector4_f32[0] * 255.0f);
        pColor[i * 2 + 1] = static_cast<uint8_t>(pColorInternal[i].vector4_f32[1] * 255.0f);
    }
}
