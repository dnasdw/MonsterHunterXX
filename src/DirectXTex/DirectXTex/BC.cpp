...
#pragma prefast(suppress:22103, "writing blocks in two halves confuses tool")
        pColorInternal[i] = XMVectorSetW(pColorInternal[i], static_cast<float>(dw & 0xf) * (1.0f / 15.0f));
    }
...
void DirectX::D3DXEncodeBC2(uint8_t *pBC, const uint8_t *pColor, uint32_t flags)
{
    ...
    for (size_t i = 0; i < NUM_PIXELS_PER_BLOCK; ++i)
    {
        ...
        uint32_t u = static_cast<uint32_t>(fAlph * 15.0f + 0.5f);
...
void DirectX::D3DXDecodeBC3(uint8_t *pColor, const uint8_t *pBC)
{
    ...
    // RGB part
    DecodeBC1(pColorInternal, &pBC3->bc1, false);
    ...
    for (size_t i = 0; i < 8; ++i, dw >>= 3)
        pColorInternal[i] = XMVectorSetW(pColorInternal[i], fAlpha[dw & 0x7]);
